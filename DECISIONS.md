...
Consequences: Contract scope for the next core implementation slice is now explicit and reviewable across API/web/shared contracts, while filtering remains intentionally deferred and auth/session behavior remains unchanged. Implementation remains a follow-up slice and is not part of this decision-only/spec-only pass.
